...
"""
Purpose:
    Deterministic real-data canary gate for OMAPS using a real split
    (prefers val, falls back to test/train). Fails on missing canaries or
    invariant violations; supports regenerating the canary list and optional
    audit exports.

Usage:
    python tivit/tests/test_dataset_omaps_real.py
    TIVIT_CANARY_REGEN=1 python tivit/tests/test_dataset_omaps_real.py
"""
...
CONFIG_PATH = REPO_ROOT / "tivit" / "configs" / "dataset" / "omaps.yaml"
CANARY_PATH = REPO_ROOT / "tivit" / "tests" / "resources" / "omaps_canaries.json"
...
def _load_config() -> dict:
    ...
    root_dir = dataset_cfg.get("root_dir", "data/omaps")
    ...
    frame_targets_cfg: Dict[str, Any] = dict(dataset_cfg.get("frame_targets", {}) or {"enable": True})
    frame_targets_cfg.setdefault("enable", True)
    frame_targets_cfg.setdefault("tolerance", 0.03)
    frame_targets_cfg.setdefault("dilate_active_frames", 1)
    frame_targets_cfg.setdefault("fill_mode", "overlap")
    frame_targets_cfg.setdefault("hand_from_pitch", True)
    frame_targets_cfg.setdefault("clef_thresholds", [60, 64])
    frame_targets_cfg.setdefault("note_min", 21)
    frame_targets_cfg.setdefault("note_max", 108)
...
def _check_determinism(ds: OMAPSDataset, idx: int, canary: Canary) -> None:
    first = ds[idx]
    second = ds[idx]
...
def _check_sampling_rules(ds: OMAPSDataset, sample: Mapping[str, Any], canary: Canary) -> None:
    T = ds.frames
...
def main() -> None:
    ...
    ds = OMAPSDataset(cfg, split=split, full_cfg=cfg)
    ...
    for canary in canaries:
        ...
        _check_targets(sample, T, canary, require_labels=STRICT_LABELS or bool(dataset_cfg.get("require_labels", True)))
        ...
        if CACHE_EQUIV:
            ...
            ds_alt = OMAPSDataset(alt_cfg, split=split, full_cfg=alt_cfg)
