...
"""
Purpose:
    Deterministic real-data canary gate for PianoVAM using a real split
...
Usage:
    ...
    TIVIT_CANARY_REGEN=1 python tivit/tests/test_dataset_pianovam_real.py
"""
...
CONFIG_PATH = REPO_ROOT / "tivit" / "configs" / "dataset" / "pianovam.yaml"
CANARY_PATH = REPO_ROOT / "tivit" / "tests" / "resources" / "pianovam_canaries.json"
...
def _load_config() -> dict:
    ...
    root_dir = dataset_cfg.get("root_dir") or "data/PianoVAM_v1.0"
...
def _check_determinism(ds: PianoVAMDataset, idx: int, canary: Canary) -> None:
    first = ds[idx]
    second = ds[idx]
...
def _check_sampling_rules(ds: PianoVAMDataset, sample: Mapping[str, Any], canary: Canary) -> None:
    T = ds.frames
...
def main() -> None:
    ...
    for canary in canaries:
        ...
        if CACHE_EQUIV:
            ...
            ds_alt = PianoVAMDataset(alt_cfg, split=split, full_cfg=alt_cfg)
