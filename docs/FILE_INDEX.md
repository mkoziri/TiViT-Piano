...
**Purpose:** Purpose:     Evaluate onset and offset predictions to determine calibrated logit or     probability thresholds, emit reliability diagnostics, and continuously write
...
| `--max-clips` | max_clips | int | `400` | Limit number of clips evaluated (default: 400) |
| `--frames` | frames | int | `64` | Frames per clip during calibration (default: 64) |
...
| `--verbose` | verbose | str | `quiet` | Logging verbosity Choices: quiet, info, debug. |
...
**Purpose:** Automated training/calibration driver for TiViT-Piano. ""Purpose:     Cycle through repeated "train → calibrate → evaluate" rounds until the
