"""Purpose:
    Provide the ViT-S/DeiT-S tile backend for TiViT-Piano. This backend reuses
    the tiled video input pipeline but replaces the ViViT-style encoder with a
    shared 2D ViT backbone, lightweight temporal transformer, and cross-tile
    mixing blocks before feeding the standard multi-task heads.

Key Functions/Classes:
    - SharedTileViTEncoder: Wraps a timm Vision Transformer with preprocessing,
      interpolation, and optional freezing so tiles of arbitrary sizes can be
      encoded with pretrained weights.
    - TileTemporalEncoder: Applies temporal self-attention independently to
      each tile sequence to inject short-term context.
    - CrossTileMixer: Lets tiles communicate at every time step and emits both
      per-tile embeddings and pooled global embeddings for the heads.
    - ViTSTilePiano: High-level module that mirrors TiViTPiano's forward
      contract so the rest of the training, calibration, and decoding stack
      stays backend-agnostic.

CLI:
    Not a CLI module; instantiate via :func:`models.build_model`.
"""
...
from .tivit_piano import MultiLayerHead
...
class SharedTileViTEncoder(nn.Module):
    ...
    def train(self, mode: bool = True) -> SharedTileViTEncoder:  # type: ignore[override]
        super().train(mode)
        # Keep the pretrained backbone deterministic when frozen.
        if self.freeze_backbone:
            self.backbone.eval()
        return self
    ...
    def forward(self, tiles: torch.Tensor) -> torch.Tensor:
        ...
        frames = rearrange(tiles, "b tt tile c hh ww -> (b tt tile) c hh ww")
        ...
        if feats.dim() == 3 and feats.shape[1] > 1:
            # timm ViT returns [cls | patch tokens]; take CLS embedding.
            feats = feats[:, 0]
        if feats.dim() != 2:
            raise ValueError(
...
class ViTSTilePiano(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Reuse temporal head count for tile mixing to keep configuration compact.
    ...
    def forward(self, x: torch.Tensor, return_per_tile: bool = False, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        tile_stack = torch.stack(tiles, dim=2)  # (B, T, tiles, C, H, W)
        ...
        # Clip-level mode: average temporal features before heads.
