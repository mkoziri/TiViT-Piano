...
def _set_onoff_head_bias(model, prior=0.01):
...
def compute_loss_frame(out: dict, batch: dict, weights: dict):
    ...
    loss_onset  = focal_on(onset_logit, onset_any)   * weights["onset"]
    loss_offset = focal_off(offset_logit, offset_any)* weights["offset"]
...
def main():
    ...
    if cfg["training"].get("eval_freq", 0):
        ...
        try:
            ...
            try:
                make_dataloader(cfg, split=cfg["dataset"].get("split_test","test"))
            except Exception:
                val_loader = None
