...
CONFIG = REPO / "configs" / "config_pianovam_fast.yaml"
TRAIN = REPO / "scripts" / "train.py"
CALIBRATE_THRESH = REPO / "scripts" / "calib" / "calibrate_thresholds_pianovam.py"
...
def apply_metrics_to_config(metrics: Dict[str, float]) -> None:
    ...
    # Write back calibrated thresholds.
    # IMPORTANT: For PianoVAM we legitimately see low best_prob values (e.g. 0.02),
    # so we must not clamp them to [0.30, 0.80] or we'll silently ignore calibration.
    lo, hi = FAST_RESULT_MIN, FAST_RESULT_MAX  # typically 0.02 .. 0.98
    ...
    if offset_prob is not None:
        if lo <= offset_prob <= hi:
            metrics_cfg["prob_threshold_offset"] = float(offset_prob)
        else:
            print(
                f"[autopilot] WARNING: offset prob_threshold={offset_prob:.4f} outside [{lo:.2f}, {hi:.2f}]; skipping write"
            )
    agg_cfg["mode"] = "k_of_p"
...
def perform_calibration(
    *,
    ...
    split_calib = _split_for_calib(split)
    split_eval  = _split_for_eval(split)
...
# Main orchestration
# ---------------------------------------------------------------------------
def _split_for_calib(split: str) -> str:
    s = (split or "").strip().lower()
    return "val" if s in {"val", "valid", "validation"} else s

def _split_for_eval(split: str) -> str:
    s = (split or "").strip().lower()
    return "valid" if s in {"val", "valid", "validation"} else s

def main() -> int:
    # normalize split names for scripts that only accept {train,val,test}
    ...
    split_eval = args.split_eval   # αν το θες σαν local alias
